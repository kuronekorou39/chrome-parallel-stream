...
(function streamControl() {
  ...
  // 親(multiview.html)からの「音量設定」指示(マスタ音量つまみ)。
  // value(0〜1)を全 video に一括適用。0 ならミュート。継続監視はしないので、
  // 以後ユーザーがプレイヤー自前で音量を変えても巻き戻さない。
  window.addEventListener('message', (e) => {
    if (e.source !== window.parent) return; // 親以外からの偽装を弾く
    const d = e.data;
    if (!d || d[MAGIC] !== true || d.type !== 'set-volume') return;
    const vol = Math.max(0, Math.min(1, Number(d.value)));
    const vids = document.querySelectorAll('video');
    for (const v of vids) {
      try { v.volume = vol; v.muted = vol <= 0; } catch (err) { /* noop */ }
    }
  });
